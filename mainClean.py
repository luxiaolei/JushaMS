...
def dfBuilder(kw, drop=True):
    ...
    if drop:
        for uidName in ['核心客户号', 'v_khdm', '客户代码']:
            try:
...
############################################################################
uimageCols = ['v_khdm', '性别', '年龄', '婚姻', '学历', '从属行业', '理财抗风险等级', '客户层级', '新老客户标记',
...
uinfoCols = ['v_khdm', '首次开户日期', '贵宾客户等级描述', '账户即时通签约标志',  '活期余额', '存款余额月日均', '存款余额年日均',
...
#index type convert
dftrade['v_khdm'] = dftrade['v_khdm'].apply(intIndex)

#drop
dftrade.drop(labels=['统计日期', 'CUST_NAME'], axis=1, inplace=True)
...
if not Consider0BalanceAsPositive:
    uid107 = dfasset.ix[(dfasset['金融资产代码']==107)&(dfasset['金融资产余额']!=0), 'v_khdm'].drop_duplicates().values
    uid170 = dfasset.ix[(dfasset['金融资产代码']==170)&(dfasset['金融资产余额']!=0), 'v_khdm'].drop_duplicates().values
    uid130 = dfasset.ix[(dfasset['金融资产代码']==130)&(dfasset['金融资产余额']!=0), 'v_khdm'].drop_duplicates().values
else:
    uid107 = dfasset.ix[dfasset['金融资产代码']==107, 'v_khdm'].drop_duplicates().values
    uid170 = dfasset.ix[dfasset['金融资产代码']==170, 'v_khdm'].drop_duplicates().values
    uid130 = dfasset.ix[dfasset['金融资产代码']==130, 'v_khdm'].drop_duplicates().values
...
for code, df in uidGrouped:
    ...
    newdf = df.ix[:, ['v_khdm', '金融资产余额_'+str(code), '金融资产余额季日均_'+str(code)]]
    newdf.drop_duplicates(subset='v_khdm', inplace=True) #user can not hold one asset with multipul records
   
    newdf.set_index('v_khdm', drop=True, inplace=True)
    dfs.append(newdf)
...
sys.stdout.flush()
############################################################################
dfuimage['Y107'] = dfuimage['v_khdm'].apply(lambda x: 1 if x in uid107 else 0)
dfuimage['Y170'] = dfuimage['v_khdm'].apply(lambda x: 1 if x in uid170 else 0)
dfuimage['Y130'] = dfuimage['v_khdm'].apply(lambda x: 1 if x in uid130 else 0)

dfuimage.set_index('v_khdm', drop= True, inplace= True)
dftrade.set_index('v_khdm', drop= True, inplace= True)
dfuinfo.set_index('v_khdm', drop= True, inplace= True)
...
dfXY = dfXY.reset_index().drop_duplicates(subset='v_khdm').set_index('v_khdm')
...
dfuimageRaw.set_index('v_khdm', drop=True, inplace =True)
dfuimageRaw.drop_duplicates(inplace=True)
dfCleaned = dfCleaned.ix[:, ['v_khdm','Y107','Y170','Y130']].join(dfuimageRaw, how='left')
print('after join uimageRaw, cleaned.csv shape:', dfCleaned.shape)

dfCleaned[['Y107', 'Y170', 'Y130']] = dfCleaned[['Y107', 'Y170', 'Y130']].astype(str)
dfCleaned['v_khdm'] = dfCleaned.index.values
dfCleaned.drop_duplicates(subset=['v_khdm'], inplace=True)
