...
Implementation of the jushacore algorithm with the following characteristics:
...
try:
    ...
    sys.stderr.write('jushacore warning: Could not load the module '
...
def jushacore(pcd, filt, cover, cutoff,
           mask=None,
    ...
    :return: jushacore output data structure
    :rtype: ``jusha_output`` instance
    '''
    # input checks
    ...
    if pcd.ndim==1:
        ...
        if not np.all(pcd>=0):
            raise ValueError('jushacore needs nonnegative dissimilarity values.')
    else:
        assert pcd.ndim == 2
        [N, n] = pcd.shape
    ...
    if verbose:
        for i in range(filt.shape[1]):
            print('jushacore: Filter range in dimension {0}: [{1:0.2f}, '
                  '{2:0.2f}]'.format(i, np.min(filt[:,i]), np.max(filt[:,i])))
        print('jushacore: Cover: {0}'.format(cover))
        print('jushacore: Clustering: {0}'.format(cluster))
        print('jushacore: Cutoff: {0}'.format(cutoff))
