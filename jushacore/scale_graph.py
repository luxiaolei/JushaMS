...
if sys.hexversion < 0x03000000:
    range = xrange
from jushacore.tools import progressreporter
...
def scale_graph(M, filt, cover=None, simple=False, **kwargs):
    ...
    #xl_add
    if not M.stopFlag:
        print('Genrating complex from {0} of nodes'.format(M.num_nodes))
...
def do_scale_graph(M, weighting='inverse', exponent=0., maxcluster=None,
    ...
    '''
    Compute the scale graph from a jusha output.
    '''
    ...
    for i in range(1,layers):
        ...
        """
        #origin error code. python3 can not compare float with Nonetype
        ...
        """
        #in python2.x, None < number, is True
        #in python3.x, it will rise error
        def xlFunc1(j):
            if type(LB1[j])== type(None):
                return False
            elif type(diam2)== type(None):
                return True
            else:
                return LB1[j]>=diam2
        def xlFunc2(j):
            if type(LB2[j])== type(None):
                return False
            elif type(diam1)== type(None):
                return True
            else:            
                return LB2[j]>=diam1
        for j in takewhile(xlFunc1, range(N1)):
            Dijkstra.add_edge(j+1,0)
        for j in takewhile(xlFunc2, range(N2)):
            Dijkstra.add_edge(0,j+1)
    ...
    sgd.path, sgd.infmin = Dijkstra.shortest_path(verbose)
    if verbose:
...
class Layered_Dijkstra():
    ...
    def shortest_path(self, verbose=True):
