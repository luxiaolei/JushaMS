# -*- coding: utf-8 -*-
'''
This file is part of the Python jusha package, an open source tool
...
Python jusha is distributed under the GPLv3 license. See the project home page

    http://danifold.net/jusha

for more information.
'''
'''
This module contains all the classes to define the L{jusha_output} class.
As the name says, it is used as a container for the output from the jusha algorithm,
and the methods for displaying and analyzing the jusha output accept this class as the input format.
'''
import sys
...
from jushacore.tools import dict_items, dict_values
from jushacore.draw_jusha_output import draw_2D, draw_scale_graph, save_scale_graph_as_pdf, \
...
class jusha_output:
    ...
    This class stores the information given by the jusha algorithm
    in the following attributes:

      - B{info}: a dictionary. This is a freeform attribute. Use it to store
        any information like the data source, the jusha parameters, the
      ...
      - B{simplices}: the higher-dimensional simplices in the jusha output,
    ...
    def __init__(self, info=None, levelsets=None, nodes=None, simplices={},
                 point_labels=None):
        '''
        Generate a new L{jusha_output} object.
    ...
    def __repr__(self):
        '''
        Represent the data of the jusha output by a string.
        '''
        return repr({
            'info':      self.info,
    ...
    def add_node(self, level, points, attribute=None):
        ...
        #xl_add, points[i] type is np.int64 which is not JSON serilizeable
    ...
    def generate_complex_new(self, cover=None,
        ...
        while S: # while S is not empty
            ...
            for i1, Si1 in S:
                t1 = time()
                if t1 - t0 > maxRunMinutes* 60:
                    self.stopFlag = True
                    print('Gen simplices costs too much time!')
                    break
    ...
    def generate_graph(self, cover=None, verbose=False, min_sizes=()):
        ...
        for i, n in enumerate(self.nodes):
            assert len(n.level)==1, \
                'This method can only be used for the 1-dimensional jusha.'
            if n.points.size>=min_nodesize:
                self.add_simplex((i,), len(n.points))
    ...
    def save_to_file(self, filename):
        '''
        Save the jusha output to a Pickle file.
    ...
    def load_from_file(filename):
        '''
        Load a jusha output structure from a Pickle file.
    ...
    def to_d3js_graph(self):
        """
        Convert the 1-skeleton of a L{jusha_output} to a dictionary
        ...
        G['vertices'] = [{'index': i, 'level': n.level, 'members':
                         ...
                         """
        #xl_add
        G['nodes'] = [{'index': v, 'group': v[0]} for v in
                      self.simplices[0]]
    ...
    def from_Graph(G):
        '''
        Convert a networkx Graph to a 1-dimensional L{jusha_output}. This
        only makes sense for graphs which have been converted from jusha
    ...
    def from_db( cursor, expr_id ):
        """
        Given a psycopg2 cursor object that points to a postgres db with the jusha_output schema
        and an expr_id, will retrieve the corresponding jusha_output object.

        @param cursor: db cursor
        @type cursor: psycopg2._psycopg.cursor

        @param expr_id: experiment id for the jusha experiment you want to retrieve from the db
        @type expr_id: int
        ...
        expr_ids, expr_info_by_expr_ids, levelsets_by_expr_ids, nodes_by_expr_ids, edges_by_expr_ids =\
            jusha_results_from_db.jusha_results_from_db( cursor, expr_ids = [expr_id] )

        M = jusha_output(info=expr_info_by_expr_ids[0],
    ...
    def nodes_from_path(self, filt):
        '''
        Apply a path through the dendrograms to a jusha output.
        '''
        if filt.ndim == 1:
        ...
        for i, (dataidx, Z, diam), num_clust in zip(count(),
            ...
            if num_clust==0:
                #xl_add changed dataidx.size  to 0
...
class scale_graph_data:
    ...
    def append(self, dataidx, dendrogram, diameter, levelindex):
        ...
        #xl_add for 2-d filter
        index = self.indexindicator
